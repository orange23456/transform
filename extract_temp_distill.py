...
docs=[('temperature_control',Path(r'C:\Users\hz-user\Desktop\温控系列产品.docx'),root/'temperature_control_source_images'),('distillation',Path(r'C:\Users\hz-user\Desktop\蒸馏系列产品.docx'),root/'distillation_source_images')]
...
for label,src,out in docs:
    ...
    for f in files:
        ...
        im.thumbnail((340,270)); canvas=Image.new('RGB',(370,330),'white')
        canvas.paste(im,((370-im.width)//2,10)); d=ImageDraw.Draw(canvas)
        d.text((10,292),f.name,fill='black'); d.text((10,310),f'{Image.open(f).size[0]}x{Image.open(f).size[1]}',fill='gray')
        thumbs.append(canvas)
    cols=3; rows=max(1,math.ceil(len(thumbs)/cols)); sheet=Image.new('RGB',(cols*370,rows*330),'white')
    for i,t in enumerate(thumbs): sheet.paste(t,((i%cols)*370,(i//cols)*330))
